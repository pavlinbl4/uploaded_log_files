...
def create_report_file():
    ...
    ws['A1'] = 'image_file_name'  # create columns names
    ws['B1'] = 'Ъ'
    ws['C1'] = 'PXP'
    ws['D1'] = 'TASS'
...
def write_to_file(path_to_file, image_file_name, column_name, line_number):
    wb = load_workbook(path_to_file)
    ws = wb["images"]
    ws[f'A{line_number}'] = image_file_name
    ws[f'{column_name}{line_number}'] = '*****'
    wb.save(path_to_file)


def find_row_with_image_name(image_file_name, path_to_report_file):  # return row number containing image_file_name
    wb = load_workbook(path_to_report_file)
    ws = wb["images"]
    for cell in ws['A']:
        if cell.value == image_file_name:
            return cell.row
    return None
...
if __name__ == '__main__':
    ...
    image_file_name = '20171024_PAV5943_test_22.JPG'
    column_name = 'B'

    path_to_report_file = create_report_file()

    row_number = find_row_with_image_name(image_file_name, path_to_report_file)
